...
import { compareRegimes, type RegimeComparisonResult } from "@/lib/tax/engine/regime-comparison";
import type { FinancialYear } from "@/lib/tax/financial-year";
import { isProfileWithinSupportedScope } from "@/lib/tax/mapping";
import { listTaxDeductions, listTaxIncomeAdjustments, listTaxPayments, listTaxWithholdings, getTaxProfile } from "@/lib/tax/queries";
import type { TaxRuleSet } from "@/lib/tax/rules/types";
import type { Database } from "@/types/database.types";

export type RegimeComparisonAvailability =
  | { available: true; result: RegimeComparisonResult; hasSalaryOrPensionIncome: boolean }
...
export async function getRegimeComparisonForYear(
  ...
  const confirmedDeductions = deductions.filter((d) => d.status === "confirmed");
  const oldRegimeDeductions = confirmedDeductions.reduce((sum, d) => {
    ...
    const capped = rule.maxAmount ? Decimal.min(d.claimedAmount, rule.maxAmount) : d.claimedAmount;
  ...
  const taxWithheld = withholdings.reduce((sum, w) => sum.plus(w.taxWithheld), new Decimal(0));
